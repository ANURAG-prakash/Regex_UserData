...
namespace ConsoleApp1
{
    class Pattens
    {
        ...
        public static string Regex_Password = "^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%>,<.&*-_+=])[a-zA-Z0-9!@#$%>,<.&*-_+=]{8,}$";
        /*
        ...
        */
        public bool ToCheckName(string name) => (Regex.IsMatch(name, Regex_Name));

        public bool ToCheckLastName(string lastName) => (Regex.IsMatch(lastName, Regex_LastName));

        public bool ToCheckMail(string mail) => (Regex.IsMatch(mail, Regex_Email));

        public bool ToCheckNumber(string number) => (Regex.IsMatch(number, Regex_Number));

        public bool ToCheckPassword(string password) => (Regex.IsMatch(password, Regex_Password));
